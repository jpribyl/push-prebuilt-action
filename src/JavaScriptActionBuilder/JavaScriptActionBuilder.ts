...
export class JavaScriptActionBuilder extends ActionBuilder {
  ...
  private async installNccGlobally() {
    ...
    await exec.exec("npm install -g @vercel/ncc");
    core.endGroup();
  }
  ...
  async buildAllEntrypoints() {
    core.debug(
      `buildAllEntrypoints: ${JSON.stringify(this.actionConfig.runs)}`
    );
    ...
    await Promise.all(
      entrypointCandidacies.map((entry) => this.buildSingleEntrypoint(entry))
    );
  }

  async buildSingleEntrypoint(entry: "pre" | "main" | "post") {
    core.debug(
      `buildSingleEntrypoint: typeof this.actionConfig ${typeof this
        .actionConfig}`
    );
    core.debug(`buildSingleEntrypoint: ${JSON.stringify(this.actionConfig)}`);
    core.debug(
      `buildSingleEntrypoint, this.actionConfig,runs: ${JSON.stringify(
        this.actionConfig.runs
      )}`
    );
    ...
    const compiled = await this.getCompiler().compile(
      this.actionConfig.runs[entry]!
    );
  ...
  shouldReplaceEntrypoint(
    entrypoint: keyof JavaScriptActionConfig["runs"]
  ): boolean {
